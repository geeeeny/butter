var replyTempl = {
	...
	mediaObjectTempl : function(reply, writer) {
		...
		<div class="media d-block d-md-flex mt-3"
			...
			<img class="d-flex mb-3 mx-auto avatar rounded-circle“
				width="80" src="/butter/member/avata?userId=${reply.writer}">
			<div class="media-body text-center text-md-left ml-md-3 ml-0"> 
				<div style="overflow:hidden"> 
					<h5 class="mt-0 font-weight-bold float-left"> 
						${reply.writer}</h5> 
					<div class="buttons float-right ml-3">
				</div> 
				<div class="float-right">작성일:${reply.regDate}</div> 
				</div> <div class="reply-content">${reply.content}</div> 
				<div class="work"></div> 
	...
	buttonsTempl : function(item, reply, writer) { //자식은 포함 안 됨(아직 DOM에 들어가지 않았기 때문)
		// 삭제 글이 아닌 경우
		if(!reply.deleted) { 
			// 로그인 사용자인경우 댓글 버튼 추가
			if( writer != '') { 
			...
			if(reply.deleted == 0 && reply.writer == writer) { 
				item.find('.buttons').append( '<button class="reply-edit-show"><i class="fa fa-edit"></i></button>'+ '<button class="reply-delete"><i class="fa fa-trash"></i></button>'); 
			} 
		} else { // 삭제된 댓글
